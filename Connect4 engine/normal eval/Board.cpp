...
Board::Board() :pieceBitboards{ 0,0 }, result{ COLOR_NONE }, gamePly{ 0 }, reachedMinimalHeight{ false }
{
	...
	m_input.resize(Board::rows, Board::cols * input_channels);
	m_input.setZero();
#endif
}
...
bool Board::doMove(int index)
{
	...
	else if (result == COLOR_NONE)
	{
		...
		for (int channel = 0; channel < (input_channels/3) && moveHistoryIndex >= 0; channel++, moveHistoryIndex--)
		{
			...
			m_input(move.row, move.col + (channel * cols * 3) + cols * 2) = 1;
			m_input(move.row, move.col + (channel * cols * 3) + (cols * sideToMove)) = 1;
		}
#endif
...
void Board::undoMove()
{
	...
	{
		const Point& move = moveHistory[moveHistoryIndex];
		m_input(move.row, move.col + (channel * cols * 3) + cols * 2) = 0;
		m_input(move.row, move.col + (channel * cols * 3) + (cols * sideToMove)) = 0;
	}
#endif
